...
class visulize():
    def base_map(self, real_data):
        ...
        for i in range(len(real_data)):
            if i == 0:
                draw.text((self.new_data[i][0] + 15, self.new_data[i][1] + 27), "DC", "black", font=self.font)
            else:
                draw.text((self.new_data[i][0] - 10, self.new_data[i][1] + 25), "sensor%d" % i, "black", font=self.font)
