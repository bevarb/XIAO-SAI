import copy
from models.new_ant_model import Ant
...
import time
def ants_method(distance_graph, car_num, Total_iter, ALPHA=1.0, BETA=2.0, RHO=0.5, Q=100.0, city_num=30, ant_num=30):
    """===============================算法参数设置==============================="""
    # 参数
    '''
    ...
    pheromone_graph = [[1.0 for col in range(city_num + car_num - 1)] for raw in range(city_num + car_num - 1)]
    """==================================搜索==================================="""
    ants = [Ant(ID, city_num, car_num, ALPHA, BETA, distance_graph, pheromone_graph) for ID in range(ant_num)]  # 初始蚁群
    best_ant = Ant(-1, city_num, car_num, ALPHA, BETA, distance_graph, pheromone_graph)  # 初始最优解
    ...
    for i in range(Total_iter):
        ...
        # 更新信息素
        # 获取每只蚂蚁在其路径上留下的信息素
        temp_pheromone = [[0.0 for col in range(city_num + car_num - 1)] for raw in range(city_num + car_num - 1)]
        for ant in ants:
            for i in range(1, city_num + car_num - 1):
...
if __name__ == "__main__":
    ...
    # 蚁群算法
    path, info, pass_time, logs = ants_method(dist, car_num=4, Total_iter=100, ALPHA=1.0, BETA=1.8, ant_num=80)
